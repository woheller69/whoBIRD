...
import com.zell_mbc.birdlog.BirdDBHelper
import com.zell_mbc.birdlog.PlayNotification
...
class SoundClassifier(
    ...
    recognitionTask = Timer().scheduleAtFixedRate(inferenceInterval, inferenceInterval) task@{
      ...
      if (mBinding.progressHorizontal.isIndeterminate){  //if start/stop button set to "running"
        probList.withIndex().also {
          ...
          updateTextView(max, mBinding.text1, timeInMillis, MainActivity.hitText1)
          updateImage(max)
          ...
          updateTextView(secondMax,mBinding.text2,timeInMillis, MainActivity.hitText2)
        }
      }
  ...
  private fun updateTextView(element: IndexedValue<Float>?, tv: TextView, timeInMillis: Long, hitText: MutableState<String>) {
    val sharedPref = PreferenceManager.getDefaultSharedPreferences(mContext)
    if (element != null && element.value > sharedPref.getInt("model_threshold", 30)/100.0) {
      ...
      Handler(Looper.getMainLooper()).post {
        ...
        hitText.value = label + "  " + Math.round(element.value * 100.0) + "%"
      }
    } else {
      hitText.value = ""
      Handler(Looper.getMainLooper()).post {
